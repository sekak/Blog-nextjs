...
const Pagination = ({ page, count, cat }) => {
  ...
  useEffect(() => {
      ...
      else if (href != "blog" && name == "prev") { // prev // no blog // right
      ...
      else if (href == "blog" && name == "next") // next //  blog // right
      {
      ...
      else if (href != "blog" && name == "next") { // next // no blog // right
  ...
  return (
    <div className={style.container}>
       <button disabled={block} className={`${style.button} ${block && style.block}`} onClick={() => setName("prev")}>Previous</button>
       <button disabled={block1} className={`${style.button} ${block1 && style.block}`} onClick={() => setName("next")}>Next</button>
     </div>
  )
}
